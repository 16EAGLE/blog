...
State:          locked read
Type:           normal (base)
Location:       /Virtualbox/VMs/vm1/sysvol_vm1.vmdk
Storage format: VMDK
Capacity:       90000 MBytes
Encryption:     disabled
```

If you are not sure which virtual hard disk is the one you are looking for, you can use `vboxmanage list vms -l` to list details of all VMs on your host, including the attached hard disks and their UUID per VM.
...
Create a directpry to which you want to mount the disk image, e.g. using:

```
sudo mkdir /mnt/sysvol_vm1
```

Then, use `vboximg-mount` with the UUID you extraced to mount the disk on the host as a FUSE-based file system:

```
vboximg-mount --image 968d86b7-eb9f-416a-bf14-e3217fe045a3 -o allow_root /mnt/sysvol_vm1
```
...
```
sysvol_vm1.vmdk  vhdd  vol0  vol1
```

* `sysvol_vm1.vmdk` is a symlink that links to the original hard disk image.
...
```
mkdir /mnt/sysvol_vm1_vol0
sudo mount /mnt/sysvol_vm1/vol0 /mnt/sysvol_vm1_vol0
```

That's it! Now you have access to the file system of the first volume on the VirtualBox hard disk image that we mounted!
